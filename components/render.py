...
class TemplateLexer(object):
    ...
    def add_simple_block_statement(self, operator, suffix):
        if not suffix:
            raise ParseException("{0} missing content".format(operator))  # type: ignore
...
if __name__ == "__main__":
    c=""
    with open (os.path.dirname(os.path.abspath(__file__))+'/HTML/%s.html' % "test",'r') as f:
        c=f.read()
    t = Template(c)
    print(t.render(ClassName="你好",Year=2,a=['a','b','c'],b=[[1,2],[3,4],[5,6]]))
